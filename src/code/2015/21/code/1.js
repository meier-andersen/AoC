export const run = (boss) => {
  const wins = findAllWins(boss);
  
  return findCheapest(wins);
};

const findCheapest = wins => {
  let cheapest = null;
  wins.forEach((win) => {
    if (!cheapest || win.cost < cheapest.cost) cheapest = win;
  });

  return cheapest.cost;
}

const findAllWins = (boss) => {
  const wins = [];
    ...
    for (let a = 0; a < armors.length; a++) {
      for (let r1 = 0; r1 < rings.length; r1++) {
        for (let r2 = 0; r2 < rings.length; r2++) {
          ...
          if (tryFight(hero, boss)) wins.push(hero);
        }
      }
    }
  }
