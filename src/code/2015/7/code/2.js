export const run = (input) => {
  ...
  const number = makeRun(structuredClone(gates), structuredClone(operations));
  const b = operations.find(x => x.gate === "b")
  if(b) 
    b.action = `${number}`;
  
  return makeRun(structuredClone(gates), structuredClone(operations));
};

const makeRun = (gates,  operations) => {
