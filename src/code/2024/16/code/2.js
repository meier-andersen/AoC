export const run = (map) => {
  ...
  const paths = findSolutions(map);
  const res = calcScore(paths);
...
const calcScore = (paths) => {
  let res = [];
  paths.forEach((path) => {
    let last = null;
    path.visited.forEach((pos) => {
      if (last) {
        const node = nodes.find((x) => x.id === last).nodes.find((x) => x.id === pos);
        node.coor.forEach((n) => {
          if (!res.find((x) => x === n)) res.push(n);
        });
      }
...
const printMap = (map, visited = []) => {
  let coordinates = [];
  visited.forEach((visit) => {
    coordinates.push(visit.split(",").map((x) => parseInt(x)));
  });
  for (let r = 0; r < map.length; r++) {
    let str = "";
    for (let c = 0; c < map[r].length; c++) {
      const curr = map[r][c];
      if (curr === "S" || curr === "E") str += curr;
      else if (coordinates.find((x) => x[0] === r && x[1] === c)) str += "O";
      else if (map[r][c] === ".") str += " ";
      else str += "█";
    }
    console.log(str);
